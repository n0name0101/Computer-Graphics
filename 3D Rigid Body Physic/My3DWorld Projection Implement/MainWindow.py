...
def main():
    ...
    pygame.display.set_caption('Cube Demo')
    clock = pygame.time.Clock()
    change = False

    # Generate 3D World
    ThreeDWorld = World3D(SCREEN_SIZE=(SCREEN_WIDTH, SCREEN_HEIGHT), camera=np.array([0, 0, 0]),
                          lookat=np.array([0, 0, -1]))
    # Add Cube
    ThreeDWorld.add_cube(Cube(x=np.array([0, 500, -2000]), size=(100, 100), SCREEN_SIZE=(SCREEN_WIDTH, SCREEN_HEIGHT)))
    ThreeDWorld.add_cube(Cube(x=np.array([0, 300, -300]), size=(100, 100), SCREEN_SIZE=(SCREEN_WIDTH, SCREEN_HEIGHT)))
    ...
    ThreeDWorld.add_cube(Cube(x=np.array([300, 0, 300]), size=(100, 100), SCREEN_SIZE=(SCREEN_WIDTH, SCREEN_HEIGHT)))
    ThreeDWorld.add_cube(
        Cube(physic=False, x=np.array([0, 0, -2000]), size=(800, 400), SCREEN_SIZE=(SCREEN_WIDTH, SCREEN_HEIGHT)))  # Terrain
    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_r:
                    change = False if change else True
                elif event.key == K_q:
        ...
        # Update World
        ThreeDWorld.step(1 / 60, change)
        # Draw World
        ThreeDWorld.draw(screen)
